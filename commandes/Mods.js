...
zokou({ nomCom: "jid", categorie: "Mods" }, async (dest, zk, commandeOptions) => {
  ...
  const { arg, ms, repondre, verifGroupe, msgRepondu, verifAdmin, superUser, auteurMessage,auteurMsgRepondu } = commandeOptions;
         ...
         if (!superUser) {
    repondre("command reserved for owner");
    return;
  }
              if(!msgRepondu) {
                jid = dest
...
zokou({
  ...
  if (!superUser) {repondre('This command is only allowed to the owner') ; return}
    if (!arg[0]) {
...
zokou({
 ...
 if(!arg || arg.length < 1) { 
  ...
  Example: mention https://static.animecorner.me/2023/08/op2.jpg image Hi, my name is Alone`) ; return}
...
The different types are audio, video, image, and sticker.
Example: mention https://telegra.ph/file/dcce2ddee6cc7597c859a.jpg image Hi, my name is Alone
